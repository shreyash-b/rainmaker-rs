    ...
    pub fn listen(&self) -> anyhow::Result<()> {
        loop {
            ...
            let response = match path_callbacks {
                Some(callbacks) => {
                    match callbacks.get(&http_request.method()) {
                        Some(callback) => callback(http_request),
                        None => {
                            HttpResponse::from_bytes("invalid method")
                        }
                    }
                },
                None => {
            ...
            req.respond(tiny_http::Response::from_data(response.get_bytes_vectored())).unwrap()
        }
    }
}
