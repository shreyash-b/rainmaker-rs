...
impl Nvs<PickleDb>{
    pub fn new(partition: NvsPartition<PathBuf>, namespace: &str) -> Result<Self, Error>{
        ...
        if namespace_path.exists(){
            db = PickleDb::load(&namespace_path, pickledb::PickleDbDumpPolicy::AutoDump, pickledb::SerializationMethod::Json)?;
        } else {
            db = PickleDb::new(&namespace_path, pickledb::PickleDbDumpPolicy::AutoDump, pickledb::SerializationMethod::Json);
        }
